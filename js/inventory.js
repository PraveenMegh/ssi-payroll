/* SSI Inventory Module - Smart Entry with AUTO STOCK UPDATE */
const SSIInventory = (() => {
  const ENTRY_TYPES = [
  ...
  async function saveEntry() {
    ...
    await updateProductStock(productId, unitId, type, qty, st);

    await SSIApp.saveState(st);
    SSIApp.toast(`✅ Entry saved: ${SSIApp.qtyFmt(qty)} ${prod?.uom||'KG'} ${type === 'IN' || type === 'OPENING' || type === 'TRANSFER_IN' ? 'added' : 'deducted'}`);
  ...
  async function updateProductStock(productId, unitId, entryType, qty, st) {
    try {
      ...
      const isOut = ['OUT', 'TRANSFER_OUT', 'ISSUE'].includes(entryType);
      const isIn  = ['IN', 'OPENING', 'TRANSFER_IN', 'ADJUST'].includes(entryType);
  ...
  async function deleteEntry(id) {
    ...
    // Find the entry before deleting to reverse stock changes
    const entry = st.inventory.find(t => t.id === id);
    if (entry) {
      // Reverse the stock change
      const isOut = ['OUT','TRANSFER_OUT','ISSUE'].includes(entry.type);
      const isIn  = ['IN','OPENING','TRANSFER_IN','ADJUST'].includes(entry.type);
      
      // Reverse: if it was IN, subtract; if it was OUT, add back
      const reverseType = isOut ? 'IN' : 'OUT';
      await updateProductStock(entry.product_id, entry.unit_id, reverseType, entry.qty, st);
    }
    
    st.inventory = st.inventory.filter(t => t.id !== id);
    await SSIApp.saveState(st);
    SSIApp.toast('Entry deleted and stock adjusted');
    ...
    try {
      ...
      for (let idx = 0; idx < rows.length; idx++) {
        ...
        if (!dateRaw || !typeRaw || !unitName || !prodName) {
          errors.push(`Row ${idx+2}: Missing required fields`); continue;
        }
        const unit = st.units.find(u => u.name.toLowerCase() === unitName.toLowerCase() && u.active);
        if (!unit) { errors.push(`Row ${idx+2}: Unit "${unitName}" not found`); continue; }
        const prod = st.products.find(p => p.name.toLowerCase() === prodName.toLowerCase() && p.active);
        if (!prod) { errors.push(`Row ${idx+2}: Product "${prodName}" not found`); continue; }
        const validTypes = ['IN','OUT','OPENING','ADJUST','TRANSFER_IN','TRANSFER_OUT','ISSUE'];
        if (!validTypes.includes(typeRaw)) { errors.push(`Row ${idx+2}: Invalid type "${typeRaw}"`); continue; }
        ...
        if (qty <= 0) { errors.push(`Row ${idx+2}: Qty is 0 — check size/count/direct`); continue; }
        ...
        // ✅ Update product stock for each imported entry
        await updateProductStock(prod.id, unit.id, typeRaw, qty, st);
  ...
  async function clearInventory() {
    ...
    SSIApp.toast(`✅ Inventory reset — ${count} entries removed, all product stock set to 0`);
    refresh(document.getElementById('page-area'));
  }
